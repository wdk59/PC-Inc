...
}else
{
	// AI 추천
	//
	//echo <"div style = 'width:auto; height:auto; align:center;'>";
	//echo "<h1 style='font-size: 20px; margin: 50px 0px 0px 50px;'>AI Recommendation</h1>";
	...
	while ($cnt < 2) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if ($row['score'] > $max_score) {
				$tmp_id = $row['product_id'];
				$tmp_row = $row;
			}
		}
		if ($tmp_id != $selected_id) {
			/*
			echo "<div id='store' style ='display: inline-block'><BR><BR>";
			echo "<form action='./review.php' method='post'>";
			echo "<input type='hidden' name='product_id' value='".$tmp_row['product_id']."'";
			echo "<input type='hidden' name='product_name' value='".$tmp_row['name']."'";
			echo "<BR><input type='image' src = '".$tmp_row['image']."' height='280' width='180'>";

			// 제목 출력
			$shortenedName = substr($tmp_row['name'], 0, 20) . '...';
			echo "<BR>".$shortenedName."<BR>";

			//가격 출력
			echo "Price: USD".$tmp_row['price']."<BR>";
			
			// 리뷰 출력
			$review_score_query = "select score from Reviews WHERE product_id = ".$tmp_id];
			$review_result = mysqli_query($conn, $review_score_query);
                	...
                	if (mysqli_num_rows($review_result)!=0) {
                        	...
                        	echo "Score: ".$tmp_row['score']." / 5.0";
                	}
			
			
                	// 태그 있으면 출력
                	if ($tmp_row['tag'] != NULL) {
                        	echo "<BR>Tags: ".$tmp_row['tag'];
                	} else { echo "<BR><BR>"; }
